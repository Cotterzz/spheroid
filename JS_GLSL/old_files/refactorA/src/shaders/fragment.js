...
#define PBLACK vec3(0.6)
#define BLACK vec3(0.0)
#define PGREY vec3(0.85)
#define PRED vec3(1.0, 0.5, 0.5)
...
vec3 getGroundColour() {
  ...
  // Court lines
  if (courtprox.x > -linewidth && courtprox.x <= 0.0 ||
      abs(courtprox.y) < linewidth ||
      courtprox.z > -linewidth && courtprox.z <= 0.0 ||
      abs(courtprox.w) < linewidth) {
    rcol = WHITE;
  }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  lights[0] = Light(vec3(1.0), vec3(ar * 0.5, 0.5, 0.6), vec3(0.0), vec3(0.0), vec2(0.0), 0.0, 0.1, true, true);
  ...
  if (goalprox < 0.0 || ZoneInsideWall) {
  ...
  if (ZoneRob) {
    ...
    if (rindex > 0) {
      ...
      vec3 SpFc = vec3(
        centre.x + SpFoffset * cos(rotation),
        centre.y + SpFoffset * sin(rotation),
        0.0
      );
      vec3 SpFl = SpFc;
      SpFl.x += SpWidth * sin(rotation);
      SpFl.y -= SpWidth * cos(rotation);
      vec3 SpFr = SpFc;
      SpFr.x -= SpWidth * sin(rotation);
      SpFr.y += SpWidth * cos(rotation);
      vec3 SpBc = vec3(
        centre.x - SpBoffset * cos(rotation),
        centre.y - SpBoffset * sin(rotation),
        0.0
      );
      vec3 SpBl = SpBc;
      SpBl.x += SpWidth * sin(rotation);
      SpBl.y -= SpWidth * cos(rotation);
      vec3 SpBr = SpBc;
      SpBr.x -= SpWidth * sin(rotation);
      SpBr.y += SpWidth * cos(rotation);
      
      float SpFD = linePointLength(vec3(uv, 0.0), vec3(SpFl.xy, 0.0), vec3(SpFr.xy, 0.0));
      float SpH = SpFheight + (SpHeight * (SpFD / SpLength));
      float inTri = sdTriangle(uv, SpFc.xy, SpBr.xy, SpBl.xy);
      float lineDist = linePointLength(vec3(uv, 0.0), vec3(SpBc.xy, 0.0), vec3(SpFc.xy, 0.0));
      ...
      if (Robs[i].ison) {
        for (int j = 0; j < NUM_LIGHTS; j++) {
          if (lights[j].casts && lights[j].ison) {
            float beam = linePointLength(Robs[i].position, lights[j].position, vec3(uv, shadowheight));
            float cradius = Robs[i].radius;
            float blur = distance(Robs[i].position, vec3(uv, shadowheight)) / 8.0;
            float shadow = 0.0;
            if (beam <= (cradius - blur)) {
              shadow = 1.0;
            } else {
  ...
  float ambiC = 0.5;
  ambientmulti *= sqrt(
    min(ambiC - (courtprox.w * ambiA), ambiB) *
    min(ambiC - courtprox.z * ambiA, ambiB) *
    min(ambiC - courtprox.y * ambiA, ambiB) *
    min(ambiC - courtprox.x * ambiA, ambiB)
  );
  ...
  if (tracting) {
    ...
    vec2 tpA = vec2(
      Robs[tracter].position.x + tdist * cosr,
      Robs[tracter].position.y + tdist * sinr
    );
    vec2 tpB = vec2(
      Robs[tracter].position.x + tdistf * cosr,
      Robs[tracter].position.y + tdistf * sinr
    );
    ...
    vec3 lcol = tcol / 3.0;
    if (tractor < lw) {
      f = smoothstep(lv, lw, tractor);
      col = mix(col + lcol, col, f);
    }
    
    if (rindex == tractee) {
      float llw = 0.4;
      vec3 lnorm = vec3(sin(mTime * 3.0), cos(mTime * 11.0), cos(mTime * 7.0));
      float ncola = sin(dot(normal, lnorm));
      float ncolb = sin(dot(normal, lnorm.yzx));
      float ncolc = sin(dot(normal, lnorm.zxy));
      if (ncolc > sin(ncola * 5.0) && ncolc < sin(ncola * 5.0) + llw) col += tcol;
      if (ncola > sin(ncolc * 5.0) && ncola < sin(ncolc * 5.0) + llw) col += tcol;
    }
  }
  
  // Post-processing
  col = contrast(col, 1.2);
  col = gammaCorrection(col, 0.95);
